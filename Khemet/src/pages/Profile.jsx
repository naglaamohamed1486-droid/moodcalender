...
export default function Profile() {
  ...
  const viewerRole = isOwnProfile ? "self" : (authUser?.role === "admin" ? "admin" : "peer");

  const availableTabs =
    viewerRole === "self" ? ["info", "places", "trips", "contributions"] :
    viewerRole === "admin" ? ["info", "trips", "contributions"] :
    ["info", "contributions"];
    ...
    <main className="profile-main">
      ...
      {!isOwnProfile && (
        <div className="pf-viewing-notice">
          ...
          {viewerRole === "admin" && user.banned && <span className="pf-viewing-banned-tag">Banned</span>}
        </div>
      )}
      ...
      <div className="pf-link">
        <div className="pf-link-cont">
          ...
          {(viewerRole === "self" || viewerRole === "admin") && (
            <Link to={viewerRole === "self" ? "/SavedTrips" : "#"} className="pf-linkat" onClick={(e) => viewerRole !== "self" && e.preventDefault()}>
              <div className="pf-linkat-data">
          ...
          <Link to={viewerRole === "self" ? "/contributions" : "#"} className="pf-linkat" onClick={(e) => viewerRole !== "self" && e.preventDefault()}>
            <div className="pf-linkat-data">
              <div className="pf-linkat-svg"><svg
                width="28px"
                height="34px"
      ...
      <div className="pf-btn-sec">
        <div className="pf-btn-cont">
          {availableTabs.includes("info") && (
            <button className={`pf-btn ${activeTab === "info" ? "active" : ""}`} onClick={() => setActiveTab("info")}>
              User info
            </button>
          )}
          {availableTabs.includes("places") && (
            <button className={`pf-btn ${activeTab === "places" ? "active" : ""}`} onClick={() => setActiveTab("places")}>
              Favorite places <span className="pf-btn-num">{user.favorites?.length || 0}</span>
            </button>
          )}
          {availableTabs.includes("trips") && (
            <button className={`pf-btn ${activeTab === "trips" ? "active" : ""}`} onClick={() => setActiveTab("trips")}>
              Saved trips <span className="pf-btn-num"> {user.savedTrips?.length || 0}</span>
            </button>
          )}
          {availableTabs.includes("contributions") && (
            <button className={`pf-btn ${activeTab === "contributions" ? "active" : ""}`} onClick={() => setActiveTab("contributions")}>
              Contributions <span className="pf-btn-num"> {visibleContributions.length}</span>
            </button>
          )}
        </div>
      </div>
        ...
        <div className="user-profile-content">
          {activeTab === "info" &&
            <div className="info-section">
              {isOwnProfile && <button className="info-sec-btn" onClick={handleOpenEdit}>Edit Profile</button>}
              <div className="info-cont">
                ...
                <div className="info-sec">
                  ...
                  <ul className="sec-details-cont">
                    ...
                    {viewerRole === "admin" && (
                      <li className="info-det"><span className="info-det-title">STATUS</span>{user.banned ? "Banned" : "Active"}</li>
                    )}
                  </ul>
                </div>
              </div>
                ...
                <div className="pf-content-section-container" >
                  <div className="pf-content-section-header" >
                    ...
                    <div className="more-places-links">
                      <Link to="/feed" className="exfav-more">Explore more</Link>
                      <Link to="/favorites" className="fav-more">View All Favorite</Link>
                    </div>
                  </div>
          ...
          {activeTab === "trips" && availableTabs.includes("trips") && (
            <div className="pf-content-section">
              {user.savedTrips?.length > 0 ? (
                <div className="pf-content-section-container" >
                  <div className="pf-content-section-header" >
                    <h3 className="title-sect-saved"> {isOwnProfile ? "My" : `${user.name}'s`} Saved Trips</h3>
                    {isOwnProfile && (
                      <div className="more-places-links">
                        <Link to="/trip-plan" className="exfav-more">Plan more</Link>
                        <Link to="/savedtrips" className="fav-more">All Saved Trips</Link>
                      </div>
                    )}
                  </div>
                  <div className="pf-trips-grid">
                    {isOwnProfile ? (
                      ...
                      <p className="pf-empty-sub">Trip details aren't available in read-only view.</p>
                    )}
                  </div>
                </div>
              ) : (
          ...
          {activeTab === "contributions" && (
            <div className="pf-content-section">
              {visibleContributions.length > 0 ? (
                <div className="pf-content-section-container">
                  <div className="pf-content-section-header">
                    <h3 className="title-sect-saved">{isOwnProfile ? "My" : `${user.name}'s`} Contributions</h3>
                    {isOwnProfile && (
                      <div className="cont-btns">
                        <Link to="/contributions" className="cont-more">Manage All</Link>
                        <Link className="profile-btns-add" to="/addplace">Add a place</Link>
                      </div>
                    )}
                  </div>
                  <div className="pf-cards-grid">
