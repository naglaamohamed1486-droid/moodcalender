...
import { setPlaceImages, uploadSingleImage } from "../../shared/utils/PicCache";
...
export default function AddPlace() {
  ...
  useEffect(() => {
    ...
    if (cityCoords) {
      mapRef.current.flyTo([cityCoords.lat, cityCoords.lng], 12, {
        duration: 1.2,
      });
    } else if (loc.coords) {
      mapRef.current.flyTo([loc.coords.lat, loc.coords.lng], 10, {
        duration: 1.2,
      });
    }
  ...
  const handleLocationSearch = async () => {
    ...
    try {
      ...
      const cityHint =
        form.city && !queryLower.includes(form.city.toLowerCase())
          ? form.city + ", "
          : "";
      const govHint =
        form.governorate && !queryLower.includes(form.governorate.toLowerCase())
          ? form.governorate + ", "
          : "";
      ...
      if (data.length === 0) {
        ...
        const matchedCity = matchedLocation?.cities.find(
          (c) =>
        ...
        if (!matchedLocation) {
          setSearchError(
            "Location pinned, but governorate not recognised — please select manually.",
          );
        } else if (!matchedCity) {
          setSearchError(
            `Governorate set to ${matchedLocation.governorate} — city not recognised, please select manually.`,
          );
        } else {
          setSearchError("");
        }
  ...
  const handleSubmit = async () => {
    if (user.banned) {
      showToast(
        "error",
        "Your account has been suspended and can't submit new places.",
      );
      return;
    }
    const e = validate();
  ...
  return (
    <div className="addplace-main">
      ...
      <div className="addplace-cont">
        <Link className="backTocontribute" to="/contributions">
        ...
        <p className="addplace-intro-p">
          Share a place with others — your discoveries help future explorers
          chart their own journeys through Egypt.
        </p>

        {user.banned && (
          <div className="add-banned-notice">
            Your account is currently suspended. You can't add new places while
            suspended.
          </div>
        )}
        ...
        <div className="add-form-container">
          ...
          <section className="add-form-section">
            ...
            <div className="add-field">
              ...
              <input
                ...
                placeholder="e.g. White Desert National Park"
              />
              {errors.title && (
            ...
            <div className="add-field">
              ...
              <input
                className={`add-input ${errors.description ? "input-error" : ""}`}
                name="description"
                value={form.description}
                onChange={handleChange}
                placeholder="One sentence that captures the place"
              />
              {errors.description && (
                <span className="add-error">{errors.description}</span>
              )}
            </div>
            ...
            <div className="add-field">
              ...
              <select
                className={`add-input add-select ${errors.category ? "input-error" : ""}`}
                name="category"
                value={form.category}
                onChange={handleChange}
              >
              ...
              {errors.category && (
                <span className="add-error">{errors.category}</span>
              )}
            </div>

            <div className="add-field">
              <label className="add-label">
                TAGS <span className="add-label-hint">press Enter to add</span>
              </label>
              <div
                className={`add-tags-input ${errors.tags ? "input-error" : ""}`}
              >
                {form.tags.map((tag) => (
                ...
                <input
                  ...
                  placeholder={
                    form.tags.length === 0 ? "nature, adventure..." : ""
                  }
                />
              </div>
            </div>
          ...
          <section className="add-form-section">
            ...
            <div className="add-field-row">
              <div className="add-field">
                ...
                <select
                  className={`add-input add-select ${errors.governorate ? "input-error" : ""}`}
                  name="governorate"
                ...
                >
                  ...
                  {LOCATIONS.map((l) => (
                    <option key={l.governorate} value={l.governorate}>
                      {l.governorate}
                    </option>
                  ))}
                </select>
                {errors.governorate && (
                  <span className="add-error">{errors.governorate}</span>
                )}
              </div>
              ...
              <div className="add-field">
                ...
                <select
                  className={`add-input add-select ${errors.city ? "input-error" : ""}`}
                  name="city"
                  value={form.city}
                  onChange={handleChange}
                ...
                {errors.city && (
                  <span className="add-error">{errors.city}</span>
                )}
              </div>
            </div>
            ...
            <div className="add-field">
              <label className="add-label">
                PIN ON MAP{" "}
                <span className="add-label-hint">
                  click the map or search below
                </span>
              </label>
          ...
          <section className="add-form-section">
            ...
            <div className="add-field">
              ...
              <div
                className={`add-image-upload ${errors.coverImage ? "input-error" : ""}`}
              ...
              />
              {errors.coverImage && (
                <span className="add-error">{errors.coverImage}</span>
              )}
            </div>

            <div className="add-field">
              <label className="add-label">
                GALLERY <span className="add-label-hint">up to 3 images</span>
              </label>
          ...
          <section className="add-form-section">
            <h2 className="add-form-section-title">
              Your Rating & Review{" "}
              <span className="add-section-optional">(optional)</span>
            </h2>

            <div className="add-field">
              <label className="add-label">
                YOUR RATING <span className="add-label-hint">0 - 5</span>
              </label>
              <input
                className="add-input"
                name="rating"
          ...
          <div className="add-form-footer">
            <Link to="/contributions" className="add-cancel-btn">
              Cancel
            </Link>
            <button
              type="button"
