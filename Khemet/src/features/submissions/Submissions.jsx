...
import SubDetails from "./Submissionsdetails";
...
async function saveSubmissionStatus(submission, newStatus, extra = {}) {
  ...
  const updatedContributions = (data.contributions || []).map((place) =>
    place.id === submission.id
      ? { ...place, status: newStatus, ...extra }
      : place,
  );
  ...
  const handleDecision = async (submission, newStatus, extra = {}) => {
    ...
    try {
      ...
      if (newStatus === "approved") {
        await setDoc(doc(db, "places", String(submission.id)), {
          ...submission,
          status: "approved",
      ...
      if (newStatus === "rejected" || newStatus === "pending") {
        try {
          await deleteDoc(doc(db, "places", String(submission.id)));
        } catch {
    ...
    <main className="Submissions">
          ...
          <div className="pf-linkat pf-linkat-pending">
            <div className="pf-linkat-data">
              <div className="pf-linkat-svg pf-linkat-svg-pending">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M8 3h8M8 21h8M8 3c0 4 4 5 4 8s-4 4-4 8M16 3c0 4-4 5-4 8s4 4 4 8" stroke="#C9A84C" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
              <div className="pf-linkat-det">
      ...
      <div className="sub-tabs">
        <div className="sub-tabs-cont">
          <button className={`sub-tab-btn ${activeTab === "pending" ? "active" : ""}`} onClick={() => setActiveTab("pending")}>
          ...
          <button className={`sub-tab-btn ${activeTab === "approved" ? "active" : ""}`} onClick={() => setActiveTab("approved")}>
          ...
          <button className={`sub-tab-btn ${activeTab === "rejected" ? "active" : ""}`} onClick={() => setActiveTab("rejected")}>
            <svg className="sub-tab-icon sub-tab-icon-stroke" width="18px" height="18px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        {loading ? (
                  ...
                  {activeTab === "rejected" && editingReasonId === place.id && (
                    <div className="sub-reject-inline">
                      ...
                      <div className="sub-reject-actions">
                        <button className="sub-btn sub-btn-reject" onClick={() => confirmEditReason(place)} disabled={!reasonDraft.trim()}>
                          Save
                        </button>
                        <button className="sub-btn sub-btn-cancel" onClick={() => setEditingReasonId(null)}>
                          Cancel
                        </button>
                      </div>
                    </div>
          ...
          <div className="pf-empty-state">
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M3 9l1.5-5h15L21 9M3 9v9a2 2 0 002 2h14a2 2 0 002-2V9M3 9h18M9 13h6" stroke="#C9A84C" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.6" />
            </svg>
