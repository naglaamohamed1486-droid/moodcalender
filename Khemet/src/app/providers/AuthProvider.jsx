...
export function AuthProvider({ children }) {
  ...
  const updateUser = async (newData) => {
    ...
    syncUserInStorage({ ...safeData, uid: user.uid });
  };
  ...
  const saveTrip = async (trip) => {
    ...
    setUser({ ...user, savedTrips: updatedTrips });
    await updateDoc(doc(db, "users", user.uid), { savedTrips: updatedTrips });
  };
  ...
  const duplicateTrip = async (index) => {
    const copy = { ...savedTrips[index], name: savedTrips[index].name + " Copy" };
  ...
  const markTripAsBooked = async (tripId) => {
    ...
    const updatedTrips = savedTrips.map((trip) =>
      trip.id === tripId ? { ...trip, booked: true } : trip
    );
    setSavedTrips(updatedTrips);
    setUser({ ...user, savedTrips: updatedTrips });
    await updateDoc(doc(db, "users", user.uid), { savedTrips: updatedTrips });
  ...
  const addBooking = async (booking) => {
    ...
    await updateDoc(doc(db, "users", user.uid), { bookings: updatedBookings });
  };
  ...
  const addPlaceToActiveTrip = (place) => {
    setActiveTrip((prev) => {
      ...
      if (lastDay.city === place.city || !lastDay.city) {
        ...
        itinerary.push({ day: itinerary.length + 1, city: place.city, places: [place] });
      }
